...
(def clojail-sandbox (sandbox secure-tester))
        ...
        (try
          (clojure.main/repl
            ...
            :init #(do (set! *compile-path* (@bindings #'*compile-path*))
                       ...
                       (set! *e (@bindings #'*e)))
            :read (if (string? code)
                    ;; TODO : Does not work yet, completely breaks entire app. No clue why.
                    (let [reader (source-logging-pushback-reader (str "(gorilla-repl.sandboxed_interruptible-eval/clojail-sandbox '" code ")") line column)]
